...
{   
    ...
    public function save($key = null, $urlVar = null)
    {
		...
		else 
		{
			...
			$type = $pieces[sizeof($pieces)-1];

			$file['name'] = date("Y-m-d_H-i",$t)."_".JFactory::getUser()->id.".".$type;
			...
			$relativePathname = JPath::clean($mediaparams->get($path, 'images/user-images/') . '/' . $file['name']);
		...
		$mailer->setSubject("מרשם חדש הועלה למערכת האתר");
		$mailer->setBody(" מרשם חדש הועלה למערכת האתר על ידי" . $current_username);
		try
		{
			$mailer->send(); 
		}
